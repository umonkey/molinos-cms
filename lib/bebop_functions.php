...
function bebop_render_object($type, $name, $theme, $data)
{
  ...
  foreach ($options as $filename) {
    if (file_exists($fullpath = $root .'/'. $filename)) {
      $data['prefix'] = '/'. dirname(dirname($filename));

      ob_start();

      if (substr($filename, -4) == '.tpl') {
        $smarty = new BebopSmarty($type == 'page');
        $smarty->template_dir = dirname($fullpath);
        ...
        $compile_id = md5($fullpath);
        $smarty->display($fullpath, $compile_id, $compile_id);

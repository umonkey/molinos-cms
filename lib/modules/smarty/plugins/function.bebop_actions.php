...
function smarty_function_bebop_actions($params, &$smarty)
{
  ...
  if (preg_match('@^/admin/node/([0-9]+)/edit/.*@', $_SERVER['REQUEST_URI'], $m)) {
    ...
    // Ссылки на ревизии.
    $revisions = mcms::db()->getResults("SELECT `r`.`rid` as `rid`, `n`.`id` as `id`, `r`.`uid` as `uid`, `u`.`login` as `login`, `r`.`created` as `created` FROM `node__rev` `r` LEFT JOIN `node` `n` ON `n`.`rid` = `r`.`rid` LEFT JOIN `node` `un` ON `un`.`id` = `r`.`uid` LEFT JOIN `node_user` `u` ON `u`.`rid` = `un`.`rid` WHERE `r`.`nid` = :nid ORDER BY `r`.`rid` DESC", array('nid' => $nid));

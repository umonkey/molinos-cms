...
class ArchiveWidget extends Widget
{
  ...
  public function onGet(array $options)
  {
    ...
    if (!empty($tmpoptions['filter']['tags']))
      $root = parent::getRealNodeId($tmpoptions['filter']['tags'][0]);
    ...
    if (($options['root'] = $root) !== null) {
      // Находим раздел.
      $result['section'] = Tagger::getInstance()->getObject($options['root']);

      if (empty($result['section']) or empty($result['section']['archive']))
        return array();

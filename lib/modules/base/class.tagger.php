...
class Tagger
{
  ...
  public function getChildrenOf($id, $count = 1000, $offset = 0)
  {
    ...
    $sql = "SELECT * FROM `node` WHERE `parent_id` {$where} ORDER BY `left` LIMIT ". intval($offset) .", ". intval($count);
    return $this->getChildrenData($sql);
  }

  // Возвращает массив пригодных к использованию данных,
  // вытягивает значения из дополнительных полей.
  public function getChildrenData($sth, $fetch_extra = true, $fetch_att = false, $fetch_acc = true, $params = null)
  {
    $result = array();
    $origsth = $sth;

    if (is_string($sth)) {
      $_data = mcms::db()->getResults($sth);
    } else {
      $_data = $sth->fetchAll(PDO::FETCH_ASSOC);
      $sth->closeCursor();
    }
    ...
    if ($fetch_att and $this->checkFiles($result)) {
      $files = $this->pdo->getResults($sql = "SELECT `r`.`tid` AS `__node_id`, `r`.`key` AS `__key`, "
        ...
        ."`f`.`rid` as `rid`, `f`.`filename` as `filename`, `f`.`filetype` as `filetype`, "
        ."`f`.`filesize` as `filesize`, `f`.`filepath` as `filepath`  FROM `node` `n` INNER JOIN `node__rel` `r` ON `r`.`nid` = `n`.`id` "
  ...
  public function countChildrenOf($id)
  {
    ...
    $sql = "SELECT COUNT(*) FROM `node` WHERE {$sub}";
  ...
  {
    $sth  = "SELECT `n`.`id` as `id`, `n`.`parent_id` as `parent_id`, "
      ."`n`.`class` as `class`, `n`.`code` as `code`, "
      ."`n`.`created` as `created`, `n`.`updated` as `updated`, "
      ."`n`.`lang` as `lang`, `n`.`rid` as `rid`, `n`.`uid` as `uid`, "
      ."`n`.`published` as `published`, `n`.`left` as `left`, "
      ."`n`.`right` as `right`, `r`.`name` as `name`, `r`.`data` as `data` "
      ."FROM `node` `n` INNER JOIN `node__rev` `r` ON `r`.`rid` = `n`.`rid` WHERE `n`.`id` = :id";

    $data = $this->getChildrenData($sth, true, $fetch_att, array(':id' => $id));
  ...
  public function getObjects(array $keys)
  {
    ...
    $sql = "SELECT * FROM `node` WHERE `id` IN (". join(', ', $keys) .")";         
    return $this->getChildrenData($sql);
  }
  ...
  {
    $sql = "SELECT `class` FROM `node` WHERE `id` = :id";
    $res = mcms::db()->getResult($sql,array(':id' => $id));          
    return $res;
  }
  ...
  public function getObjectTree($root, $published = 0)
  {
    ...
    $sql = "SELECT `n`.`id` as `id`, `n`.`code` as `code`, `n`.`parent_id` AS `parent_id`, "
      ."`n`.`class` AS `class`, `n`.`published` AS `published`, "
      ."`n`.`deleted` AS `deleted`, `r`.`name` AS `name`, `r`.`data` AS `data` "
  ...
  private function nodeSaveRevision(array &$node, $forcerev = null)
  {
    ...
    try {
      ...
      if (empty($node['created']))
        $node['created'] = "'".$this->getUTCTime()."'";

      $node['updated'] = "'".$this->getUTCTime()."'";
  ...
  private function rowUpdate($table, $key, array $data, array $filter = null, $replace = false)
  {
    ...
    foreach ($data as $k => $v) {
      if (self::isSafeDate($k, $v)) {
        $v = str_replace("'", '', $v); //Если строка пришла в апострофах, то удаляем их
        $pair = "`{$k}` = '{$v}'";  // т.к. тут апострофы добавляются
        unset($data[$k]);
      } else {
        $pair = "`{$k}` = :{$k}";
      }
  ...
  public function getDocuments(array $ids)
  {
    ...
    if (!empty($ids)) {
      ...
      $sql = "SELECT * FROM `node` WHERE ". join(" AND ", $where);

      $result = $this->getChildrenData($sql);
    }

...
function fix_file_previews()
{
  $('.attachment-control').each(function (i) {
    var fid = $('span', this).html();

    if (fid) {
      $('div.preview', this).css('background-image',
        'url(./?q=attachment.rpc&fid='+ fid +',50,50,cw)');
    } else {
      $('div.preview', this).css('display', 'none');
    }
  });
}

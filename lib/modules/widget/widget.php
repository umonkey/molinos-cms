...
// Используется при гашении виджета.
...
// Интерфейс, используемый административными виджетами.
...
abstract class Widget implements iWidget
{
  ...
  // Разворачиваем алиасы.
  public static function getRealNodeId($nid)
  {
    if (!empty($nid) and !is_numeric($nid))
      $nid = PDO_Singleton::getInstance()->getResult("SELECT `id` FROM `node` WHERE `code` = :code", array(':code' => $nid));
    return empty($nid) ? null : $nid;
  }

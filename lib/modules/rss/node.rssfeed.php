...
class RssfeedNode extends Node
{
  public function formGet($simple = false)
  {
    ...
    $form->title = $this->id ? t('Добавление нового RSS потока') : t('Настройка RSS потока');
    return $form;
  }
  ...
  public function getDefaultSchema()
  {
    return array(
      ...
      'description' => t('Описание RSS потока, формируемого сайтом.'),
      'lang' => 'ru',
      'fields' => array(
        'name' => array(
          'label' => 'Имя потока',
        ...
        'contentfields' => array(
          ...
          'type' => 'TextLineControl',
          'required' => false,
          'description' => t('Имя одного или нескольких полей (через запятую), содержимое которых следует использовать в качестве текста записи.  Несколько значений обычно следует вводить только в том случае, если поток содержит документы разных типов, и поля с текстовым содержимым называются по-разному.  Если это поле не заполнять, записи будут отдаваться без текста (только заголовки).'),

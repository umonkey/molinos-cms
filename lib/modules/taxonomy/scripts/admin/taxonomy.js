$(function () {
	...
	if ($('form#nodeList ul.nodes').length) {
		var globalEC = $('<span class="fakelink expand-collapse">Развернуть/свернуть все</span>').toggle(function() {
			$('form#nodeList ul.nodes span.expand-collapse').removeClass('collapsed');
			$('form#nodeList ul.nodes ul').show();
			return false;
		}, function() {
			$('form#nodeList ul.nodes span.expand-collapse').addClass('collapsed');
			$('form#nodeList ul.nodes ul').hide();
			return false;
		});
		$('.nodes-controls-advanced').prepend(globalEC);
		...
		$(window).unload(function() {
			...
			$('form#nodeList ul.nodes li:has(>ul:visible)').each(function() {
				expandedTags.push($(this).find('>span.actions input[name="nodes[]"]').val() );
			});

$(document).ready(function () {
  // Красивое добавление.
  // FIXME: быстрое добавление отключено, т.к. надо
  // рендерить строки таблицы, итд
  $('#todosubmitformXX').submit(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == 'created') {
          $('table.todolist.open').prepend(data.html);
  ...
  // Быстрое управление.
  $('table.todolist input').change(todo_on_change);

  // Быстрое удаление.
  $('table.todolist .delete').click(function () {
    var nid = $(this).parent().children('.checkbox').attr('value');

    $.ajax({
      type: 'GET',
      url: '?q=nodeapi.rpc&action=delete&node='+ nid,
      dataType: 'json',
...
{
  $('p.todolist.open').css('display', $('table.todolist.open input').length ? 'none' : '');
  $('p.todolist.closed').css('display', $('table.todolist.closed input').length ? 'none' : '');
  $('h3.todolist.closed').css('display', $('table.todolist.closed input').length ? '' : 'none');
...
function todo_on_change()
{
  ...
  $.ajax({
    type: 'POST',
    url: '?q=todo.rpc&action=toggle&id='+ taskid,
    ...
    success: function (data) {
      if (data.status != 'ok') {
        ...
        var sel = 'table.todolist.'+ data.state;
    ...
    complete: function() {
      $('table.todolist .checkbox').attr('disabled', '');
      todo_display_magic();
   }
  });
}

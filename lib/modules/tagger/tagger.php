...
class Tagger
{
    ...
    // Создаёт недостающие индексы для класса.
    public function checkIndexes($class, $reindex = false)
    {
      if (!$this->hasClass($class))
        return;
      ...
      if (!empty($schema['fields'])) {
        foreach ($schema['fields'] as $field => $info) {
          ...
          $def = array(
            'type' => call_user_func(array($info['type'], 'getSQL')),
            'required' => !empty($info['required']),
            'indexed' => !empty($info['indexed']),
            'unique' => !empty($info['unique']),
            );

          if (empty($def['type']))
            continue;

          if (mcms_ctlname($info['type']) == 'NumberControl' and !empty($info['unsigned']))
            $def['type'] = 'int(10) unsigned';

          if (array_key_exists('default', $info) and $info['default'] !== '')
            $def['default'] = $info['default'];

          $table['columns'][$field] = $def;
        }
      }
      ...
      // Применяем изменения, сохраняем статус.
      $changes = (count($table['columns'])) && InfoSchema::getInstance()->checkTable('node_'. $class, $table);

      // Проиндексируем документы без раширенных свойств.
      while ($this->reindexLost($class))
        ;

      if (!$changes and !$reindex)
        return;
      ...
      foreach ($schema['fields'] as $field => $info) {
        ...
        */
        ...
        $nodes = Node::find(array('class' => $class, $field => $value));

        foreach ($nodes as $node)
          $node->save(false, $node->rid);
      }

      return true;
    }

    // Индексирует небольшую порцию документов, у которых отсутствуют расширенные данные.
    private function reindexLost($class)
    {
      $nids = $this->pdo->getResultsV("id", $sql = "SELECT `n`.`id`, `e`.`rid` FROM `node` `n` INNER JOIN `node__rev` `r` ON `r`.`rid` = `n`.`rid` LEFT JOIN `node_{$class}` `e` ON `e`.`rid` = `n`.`rid` WHERE `e`.`rid` IS NULL AND `n`.`class` = :class LIMIT 10", array('class' => $class));

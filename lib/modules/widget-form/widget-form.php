...
class FormWidget extends Widget
{
  ...
  {
    return array(
      'name' => 'Форма для создания контента',
      'description' => 'Возвращает форму для создания контента произвольного типа, если пользователь имеет на это право.',
      );
  }
  ...
  private function getTypeList($root = null)
  {
    ...
    if (null !== $root and !is_numeric($root))
      $root = parent::getRealNodeId($root);
    ...
    $sql = "SELECT `v`.`name`, `t`.`title` FROM `node` `n` "
      ...
      ."AND `t`.`hidden` = 0 "
      ."AND `t`.`internal` = 0 "
      .($this->showall ? "" : "AND ({$all} OR {$current}) ")
      ."ORDER BY `t`.`title` "
      ;
